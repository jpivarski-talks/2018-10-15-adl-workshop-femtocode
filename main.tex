...
\normalsize
problem by forbidding \mintinline{python}{infinite_loop()}. That is, these are non-Turing complete languages--- every function returns a value.
...
\vspace{0.5 cm}
\textcolor{darkblue}{\Large {\bf R}ecords:} one ``content'' for each field. Call it a ``table,'' which becomes a ``jagged table'' inside a jagged array; some contents may also be jagged or unions.
...
Array programming requires many temporary variables, which introduces an accidental complexity of its own (look at a LIGO analysis!).
\end{columns}
